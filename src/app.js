...
const PestSelector = ({ selectedPests, onPestChange }) => {
  ...
  return (
    <div className="mb-4">
      <label className="block mb-2 font-medium text-gray-700">Select Pests</label>
      <select 
        multiple 
        ...
        {crops.map(crop => {
          ...
          for (let i = 0; i < cropCells && cellsUsed < totalCells; i++) {
            cells.push(
              ...
              >
                {cropIcons[crop.name.toLowerCase()] || cropIcons.default}
              </div>
            );
            cellsUsed++;
          }
...
React.useEffect(() => {
  ...
  const fetchMapImage = () => {
    ...
    console.log('Fetching map image from URL:', url);
    setMapImage(url);
  };
...
<div className="border-0 border-dashed border-gray-300 rounded-lg h-[calc(100vh-200px)] flex items-center relative">
              {!mapImage && (
                //<img src={mapImage} alt="Farm satellite view" className="w-full h-full object-cover absolute" />
             
                <p className="text-gray-500 bg-opacity-75 p-4 bg-white">Enter a location and click "Get Map" to see a satellite view.</p>
              )}
              <div className="relative z-10 text-center  bg-opacity-75 p-4 ">
              
                {growZone && <p className="font-semibold">Grow Zone: {growZone}</p>}
              </div>
              </div>

            <div className="border-0 border-dashed border-gray-000 rounded-lg h-[calc(100vh-200px)] flex items-center ">
              {farmData.totalArea > 0 ? (
                ...
                <p className="text-gray-500 bg-opacity-75 p-4 bg-white">Enter farm data to see visualization</p>
              )}
    ...
    <div className="mt-8 bg-white shadow rounded-lg p-6" style={{ width: '95%', margin: '0 auto' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4" id="recommendationsContainer">
        {/* RecommendationCards will be appended here */}
          ...
          <div style={{ width: '30%' }} className="p-6 overflow-y-auto bg-white">
